...
def delete(id):
    ...
    if result>0:
        ...
        flash("Bu hikayeyi silme işlemi için yetkiniz yoktur.","danger")
        return redirect(url_for("index"))
#Hikaye güncelle
...
def update(id):
    if request.method=="GET":
        ...
        if result==0:
            flash("Böyle bir hikaye yok veya bu işlemi yapmaya yetkiniz yoktur.","danger")
            return redirect(url_for("index"))
        else:
            article=cursor.fetchone()
            form=ArticleForm()
...
class ArticleForm(Form):
    title=StringField("Hikaye Başlığı",validators=[validators.Length(min=5,max=100)])
    content=TextAreaField("Hikaye İçeriği",validators=[validators.Length(min=10)])
